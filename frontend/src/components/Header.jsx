...
const AnimatedHamburgerIcon = ({ isOpen }) => (
  <div className="w-6 h-6 flex flex-col justify-center items-center">
    <div className={`w-6 h-0.5 bg-current transform transition-all duration-300 ease-out ${isOpen ? 'rotate-45 translate-y-1.5' : ''}`} />
    <div className={`w-6 h-0.5 bg-current transform transition-all duration-300 ease-out mt-1 ${isOpen ? 'opacity-0' : ''}`} />
    <div className={`w-6 h-0.5 bg-current transform transition-all duration-300 ease-out mt-1 ${isOpen ? '-rotate-45 -translate-y-1.5' : ''}`} />
  </div>
);
...
const Header = () => {
  ...
  const handleResumeDownload = (option) => {
    if (option.format === 'view') {
      window.open(option.url, '_blank');
    } else {
      const link = document.createElement('a');
      link.href = option.url;
      link.download = `naveen-agarwal-resume.${option.format}`;
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled ? 'bg-gray-900/95 backdrop-blur-xl border-b border-gray-800/50 shadow-2xl' : 'bg-transparent'}`}>
          ...
          <div className="hidden md:flex items-center">
            <div className="relative">
              <button
                ...
                className="group relative bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white px-6 py-2.5 rounded-xl font-medium transition-all duration-300 transform hover:scale-105 hover:shadow-lg hover:shadow-blue-500/25 flex items-center gap-2 overflow-hidden"
              >
              ...
              <div className={`absolute right-0 mt-2 w-64 bg-gray-900/95 backdrop-blur-xl border border-gray-700/50 rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 transform origin-top-right ${isResumeOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 -translate-y-2 pointer-events-none'}`}>
                <div className="p-2">
          ...
          <div className="px-2 pt-4 pb-6 space-y-2 bg-gray-900/95 backdrop-blur-xl rounded-2xl mt-4 border border-gray-800/50 shadow-2xl">
            {navItems.map((item, index) => {
              ...
              return (
                <button
                  key={item.id}
                  onClick={() => scrollToSection(item.id)}
                  className={`flex items-center gap-3 text-gray-300 hover:text-white px-4 py-3 rounded-xl text-base font-medium w-full text-left transition-all duration-300 hover:bg-blue-500/10 hover:scale-105 transform ${isOpen ? 'translate-x-0 opacity-100' : '-translate-x-4 opacity-0'}`}
            ...
            <div className="pt-4 border-t border-gray-800/50">
              <div className="relative z-10">
                ...
                <div className={`mt-2 bg-gray-900/95 backdrop-blur-xl border border-gray-800/50 rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 ${isMobileResumeOpen ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0 pointer-events-none'}`}>
                  <div className="py-2">
                    {resumes.map((option, index) => (
                      <button
                        key={option.name}
                        onClick={() => handleResumeDownload(option)}
                        className="w-full text-left px-4 py-2.5 text-gray-300 hover:text-white hover:bg-blue-500/10 rounded-lg transition-all duration-200 flex items-center gap-3 group"
                        style={{ transitionDelay: isMobileResumeOpen ? `${index * 100}ms` : '0ms' }}
                      >
